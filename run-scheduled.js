...
async function imageToBase64(item) {
...
async function normalizeScheduledImages(items) {
  ...
  return await Promise.all(items.map(async (item) => {
    if (isHttpUrl(item) ||
      ...
      return await imageToBase64(item);
    }
...
export default async function handler(req, res) {
  ...
  try {
    const job = req.body || {};
    if (!job.id || !job.caption) {
      return res.status(400).json({ ok:false, error:"予約データが正しくありません" });
    }
